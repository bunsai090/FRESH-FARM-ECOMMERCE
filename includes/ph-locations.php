<?php
$philippineLocations = [
  "Metro Manila" => [
    "display" => "Metro Manila (NCR)",
    "cities" => [
      "Manila" => "1000",
      "Quezon City" => "1100",
      "Makati" => "1200",
      "Pasig" => "1600",
      "Taguig" => "1630",
      "Parañaque" => "1700",
      "Las Piñas" => "1740",
      "Mandaluyong" => "1550",
      "San Juan" => "1500",
      "Caloocan" => "1400",
      "Pasay" => "1300",
      "Muntinlupa" => "1780",
      "Marikina" => "1800",
      "Valenzuela" => "1440",
      "Navotas" => "1485",
      "Malabon" => "1470",
      "Pateros" => "1620"
    ]
  ],
  "CALABARZON" => [
    "display" => "CALABARZON (Region IV-A)",
    "cities" => [
      "Bacoor" => "4102",
      "Dasmariñas" => "4114",
      "Imus" => "4103",
      "Cabuyao" => "4025",
      "Santa Rosa" => "4026",
      "Biñan" => "4024",
      "Calamba" => "4027",
      "San Pedro" => "4023",
      "Lucena" => "4301",
      "Antipolo" => "1870",
      "Taytay" => "1920",
      "Cainta" => "1900"
    ]
  ],
  "Central Luzon" => [
    "display" => "Central Luzon (Region III)",
    "cities" => [
      "Angeles" => "2009",
      "Olongapo" => "2200",
      "San Fernando" => "2000",
      "Balanga" => "2100",
      "Cabanatuan" => "3100",
      "Gapan" => "3105",
      "Malolos" => "3000",
      "Meycauayan" => "3020",
      "San Jose del Monte" => "3023",
      "Tarlac City" => "2300"
    ]
  ],
  "Ilocos Region" => [
    "display" => "Ilocos Region",
    "cities" => [
      "Laoag" => "2900",
      "Vigan" => "2700",
      "San Fernando (La Union)" => "2500",
      "Dagupan" => "2400",
      "Alaminos" => "2404",
      "Urdaneta" => "2428"
    ]
  ],
  "Cordillera Administrative Region" => [
    "display" => "Cordillera Administrative Region",
    "cities" => [
      "Baguio" => "2600",
      "Tabuk" => "3800",
      "Bangued" => "2800",
      "La Trinidad" => "2601"
    ]
  ],
  "Cagayan Valley" => [
    "display" => "Cagayan Valley",
    "cities" => [
      "Tuguegarao" => "3500",
      "Ilagan" => "3300",
      "Cauayan" => "3305",
      "Santiago" => "3311",
      "Bayombong" => "3700",
      "Solano" => "3709"
    ]
  ],
  "Bicol Region" => [
    "display" => "Bicol Region",
    "cities" => [
      "Legazpi" => "4500",
      "Naga" => "4400",
      "Iriga" => "4431",
      "Sorsogon City" => "4700",
      "Tabaco" => "4511",
      "Ligao" => "4504"
    ]
  ],
  "Western Visayas" => [
    "display" => "Western Visayas",
    "cities" => [
      "Iloilo City" => "5000",
      "Bacolod" => "6100",
      "Roxas City" => "5800",
      "Kabankalan" => "6111",
      "Bago" => "6101",
      "San Carlos (Negros Occidental)" => "6127"
    ]
  ],
  "Central Visayas" => [
    "display" => "Central Visayas",
    "cities" => [
      "Cebu City" => "6000",
      "Lapu-Lapu" => "6015",
      "Mandaue" => "6014",
      "Tagbilaran" => "6300",
      "Toledo" => "6038",
      "Danao" => "6004",
      "Dumaguete" => "6200"
    ]
  ],
  "Eastern Visayas" => [
    "display" => "Eastern Visayas",
    "cities" => [
      "Tacloban" => "6500",
      "Ormoc" => "6541",
      "Baybay" => "6521",
      "Catbalogan" => "6700",
      "Borongan" => "6800",
      "Maasin" => "6600"
    ]
  ],
  "Zamboanga Peninsula" => [
    "display" => "Zamboanga Peninsula",
    "cities" => [
      "Zamboanga City" => "7000",
      "Pagadian" => "7016",
      "Dipolog" => "7100",
      "Dapitan" => "7101",
      "Isabela City" => "7300"
    ]
  ],
  "Northern Mindanao" => [
    "display" => "Northern Mindanao",
    "cities" => [
      "Cagayan de Oro" => "9000",
      "Iligan" => "9200",
      "Malaybalay" => "8700",
      "Valencia" => "8709",
      "Ozamiz" => "7200"
    ]
  ],
  "Davao Region" => [
    "display" => "Davao Region",
    "cities" => [
      "Davao City" => "8000",
      "Tagum" => "8100",
      "Panabo" => "8105",
      "Digos" => "8002",
      "Mati" => "8200"
    ]
  ],
  "SOCCSKSARGEN" => [
    "display" => "SOCCSKSARGEN",
    "cities" => [
      "General Santos" => "9500",
      "Koronadal" => "9506",
      "Kidapawan" => "9400",
      "Tacurong" => "9800",
      "Cotabato City" => "9600"
    ]
  ],
  "Caraga" => [
    "display" => "Caraga",
    "cities" => [
      "Butuan" => "8600",
      "Surigao City" => "8400",
      "Bislig" => "8311",
      "Bayugan" => "8502"
    ]
  ],
  "Bangsamoro Autonomous Region in Muslim Mindanao" => [
    "display" => "Bangsamoro Autonomous Region in Muslim Mindanao",
    "cities" => [
      "Marawi" => "9700",
      "Lamitan" => "7302",
      "Jolo" => "7400",
      "Bongao" => "7500"
    ]
  ],
  
  "Zamboanga del Sur" => [
    "display" => "Zamboanga del Sur",
    "cities" => [
      "Zamboanga City" => "7000",
      "Pagadian City" => "7016",
      // Add more cities with their postal codes
    ]
  ],
  // Add more regions and cities
];
?>