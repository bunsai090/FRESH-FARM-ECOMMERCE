...
<body>
    ...
    <main class="product-grid">
        ...
        $sql = "SELECT id, name, description, price, image_path, stock, status FROM products WHERE category = 'Vegetables'";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            // Loop through vegetables and display them
            while ($vegetable = $result->fetch_assoc()) {
                $isOutOfStock = ($vegetable['status'] === 'Out of Stock' || $vegetable['stock'] <= 0);
                
                // Determine if product should be displayed as "Low Stock"
                $isEffectivelyLowStock = !$isOutOfStock &&
                                         ($vegetable['status'] === 'Low Stock' ||
                                          ($vegetable['stock'] > 0 && $vegetable['stock'] <= LOW_STOCK_THRESHOLD));

                echo "<div class='product-card' data-product-id='" . htmlspecialchars($vegetable['id']) . "'>";
                ...
                $imagePath = '/fresh1/' . htmlspecialchars($vegetable['image_path']);
                echo "<img src='" . $imagePath . "' alt='" . htmlspecialchars($vegetable['name']) . "'>";
                echo "<h3>" . htmlspecialchars($vegetable['name']) . "</h3>";
                echo "<p class='description'>" . htmlspecialchars($vegetable['description']) . "</p>";
                ...
                if ($isOutOfStock) {
                    ...
                    echo "<span class='stock-left'>(" . htmlspecialchars($vegetable['stock']) . " left)</span>";
                } else { // In Stock (and not Out of Stock or Low Stock)
                    echo "<span class='status-badge status-in-stock'>In Stock</span>";
                    echo "<span class='stock-left'>(" . htmlspecialchars($vegetable['stock']) . " available)</span>";
                }
                echo "</div>";

                echo "<p class='price' data-price='" . htmlspecialchars($vegetable['price']) . "'>₱" . number_format($vegetable['price'], 2) . "</p>";
                ...
                echo "</div>";
                echo "<p class='total-price'" . ($isOutOfStock ? " style='display:none;'" : "") . ">Total: ₱" . number_format($vegetable['price'], 2) . "</p>";
                echo "<button class='add-to-cart-btn'" . ($isOutOfStock ? " disabled" : "") . ">" . ($isOutOfStock ? "Out of Stock" : "Add to Cart") . "</button>";
                echo "</div>";
            }
        } else {
            echo "<p style='text-align:center; grid-column: 1 / -1;'>No vegetables found in this category.</p>";
        }
