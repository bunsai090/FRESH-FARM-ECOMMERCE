...
<body>
    <header>
        ...
        <h1 class="category-title">Meat Products</h1>
        <a href="/fresh1/user-dashboard/cart/cart.php" class="nav-button">Go to Cart</a>
    </header>

    <main class="product-grid">
        <?php
        // Fetch meat products from the database
        $sql = "SELECT id, name, description, price, image_path, stock, status FROM products WHERE category = 'Meat'";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            // Loop through meat products and display them
            while ($meat = $result->fetch_assoc()) {
                $isOutOfStock = ($meat['status'] === 'Out of Stock' || $meat['stock'] <= 0);
                
                // Determine if product should be displayed as "Low Stock"
                $isEffectivelyLowStock = !$isOutOfStock &&
                                         ($meat['status'] === 'Low Stock' ||
                                          ($meat['stock'] > 0 && $meat['stock'] <= LOW_STOCK_THRESHOLD));

                echo "<div class='product-card' data-product-id='" . htmlspecialchars($meat['id']) . "'>";
                ...
                $imagePath = '/fresh1/' . htmlspecialchars($meat['image_path']);
                echo "<img src='" . $imagePath . "' alt='" . htmlspecialchars($meat['name']) . "'>";
                echo "<h3>" . htmlspecialchars($meat['name']) . "</h3>";
                echo "<p class='description'>" . htmlspecialchars($meat['description']) . "</p>";
                ...
                if ($isOutOfStock) {
                    ...
                    echo "<span class='stock-left'>(" . htmlspecialchars($meat['stock']) . " left)</span>";
                } else { // In Stock (and not Out of Stock or Low Stock)
                    echo "<span class='status-badge status-in-stock'>In Stock</span>";
                    echo "<span class='stock-left'>(" . htmlspecialchars($meat['stock']) . " available)</span>";
                }
                echo "</div>";

                echo "<p class='price' data-price='" . htmlspecialchars($meat['price']) . "'>₱" . number_format($meat['price'], 2) . "</p>";
                ...
                echo "</div>";
                echo "<p class='total-price'" . ($isOutOfStock ? " style='display:none;'" : "") . ">Total: ₱" . number_format($meat['price'], 2) . "</p>";
                echo "<button class='add-to-cart-btn'" . ($isOutOfStock ? " disabled" : "") . ">" . ($isOutOfStock ? "Out of Stock" : "Add to Cart") . "</button>";
                echo "</div>";
            }
        } else {
            echo "<p style='text-align:center; grid-column: 1 / -1;'>No meat products found in this category.</p>";
        }
