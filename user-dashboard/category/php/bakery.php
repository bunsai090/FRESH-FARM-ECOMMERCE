...
<body>
    <header>
        ...
        <h1 class="category-title">Bakery Products</h1>
        <a href="/fresh1/user-dashboard/cart/cart.php" class="nav-button">Go to Cart</a>
    </header>

    <main class="product-grid">
        <?php
        // Fetch bakery products from the database
        $sql = "SELECT id, name, description, price, image_path, stock, status FROM products WHERE category = 'Bakery'";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            // Loop through bakery products and display them
            while ($bakery = $result->fetch_assoc()) {
                $isOutOfStock = ($bakery['status'] === 'Out of Stock' || $bakery['stock'] <= 0);
                
                // Determine if product should be displayed as "Low Stock"
                $isEffectivelyLowStock = !$isOutOfStock &&
                                         ($bakery['status'] === 'Low Stock' ||
                                          ($bakery['stock'] > 0 && $bakery['stock'] <= LOW_STOCK_THRESHOLD));

                echo "<div class='product-card' data-product-id='" . htmlspecialchars($bakery['id']) . "'>";
                ...
                $imagePath = '/fresh1/' . htmlspecialchars($bakery['image_path']);
                echo "<img src='" . $imagePath . "' alt='" . htmlspecialchars($bakery['name']) . "'>";
                echo "<h3>" . htmlspecialchars($bakery['name']) . "</h3>";
                echo "<p class='description'>" . htmlspecialchars($bakery['description']) . "</p>";
                ...
                if ($isOutOfStock) {
                    ...
                    echo "<span class='stock-left'>(" . htmlspecialchars($bakery['stock']) . " left)</span>";
                } else { // In Stock (and not Out of Stock or Low Stock)
                    echo "<span class='status-badge status-in-stock'>In Stock</span>";
                    echo "<span class='stock-left'>(" . htmlspecialchars($bakery['stock']) . " available)</span>";
                }
                echo "</div>";

                echo "<p class='price' data-price='" . htmlspecialchars($bakery['price']) . "'>₱" . number_format($bakery['price'], 2) . "</p>";
                ...
                echo "</div>";
                echo "<p class='total-price'" . ($isOutOfStock ? " style='display:none;'" : "") . ">Total: ₱" . number_format($bakery['price'], 2) . "</p>";
                echo "<button class='add-to-cart-btn'" . ($isOutOfStock ? " disabled" : "") . ">" . ($isOutOfStock ? "Out of Stock" : "Add to Cart") . "</button>";
                echo "</div>";
            }
        } else {
            echo "<p style='text-align:center; grid-column: 1 / -1;'>No bakery products found in this category.</p>";
        }
