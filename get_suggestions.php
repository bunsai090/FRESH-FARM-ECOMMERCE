...
if (strlen($query) >= 2) {
    try {
        ...
        while ($row = $result->fetch_assoc()) {
            // Handle image path - make it relative to the root
            if (!empty($row['image_path'])) {
                ...
                if (!str_starts_with($row['image_path'], 'assets/')) {
                    $row['image_path'] = 'assets/images/products/' . $row['image_path'];
                }
            } else {
                $row['image_path'] = 'assets/images/products/default-product.jpg';
            }
